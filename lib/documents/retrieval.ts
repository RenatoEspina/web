...
function normalizedScores(scores: Array<number | undefined>): Array<number | undefined> {
  const valid = scores.filter((score): score is number => score !== undefined);
  if (valid.length === 0) return scores.map(() => undefined);

  const minimum = Math.min(...valid);
  const maximum = Math.max(...valid);
  if (maximum - minimum < Number.EPSILON) {
    return scores.map((score) => score === undefined ? undefined : score > 0 ? 1 : 0);
  }

  return scores.map((score) => score === undefined ? undefined : (score - minimum) / (maximum - minimum));
}
...
async function rankChunks(query: string, chunks: DocumentChunk[]): Promise<RankedChunks> {
  ...
  const lexicalNormalized = normalizedScores(lexicalScores).map((score) => score ?? 0);
  ...
  const semanticNormalized = normalizedScores(semanticScores).map((score) => score ?? 0);
  ...
  const ranked = chunks
    .map((chunk, index) => {
      ...
      const score = embeddingUsed
        ? (hasSemanticScore
          ? semanticWeight * semanticNormalized[index] + lexicalWeight * lexicalNormalized[index]
          : lexicalWeight * lexicalNormalized[index])
        : lexicalScores[index];
