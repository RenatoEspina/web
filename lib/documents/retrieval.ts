...
function scoreLexically(query: string, chunks: DocumentChunk[]): number[] {
  ...
  return chunks.map((chunk, index) => {
    ...
    const lexical = queryTerms.reduce((total, term) => {
      ...
      const idf = Math.log(
        (chunks.length + 1) / ((documentFrequency.get(term) ?? 0) + 0.5) + 1,
      );
    ...
    const phraseBonus =
      normalizedQuery.length > 5 && normalizedChunk.includes(normalizedQuery)
        ? 1
        : 0;
...
async function rankChunks(
  ...
  const semanticScores: Array<number | undefined> = chunks.map(
    () => undefined,
  );

  let embeddingUsed = false;

  const hasStoredEmbeddings = chunks.some(
    (chunk) =>
  ...
  if (hasStoredEmbeddings) {
    try {
      ...
      if (queryEmbedding) {
        ...
        embeddingUsed = semanticScores.some(
          (score) => score !== undefined,
        );
      }
    } catch (error) {
      console.error(
        "[llm-bridge] Semantic retrieval failed; using lexical retrieval",
        error,
      );
    }
  }
  ...
  /*
   * Se combinaban las señales mediante normalización min-max.
   * Eso podía amplificar pequeñas diferencias de similitud y permitir
   * que un distractor con muchas coincidencias léxicas dominara.
   *
   * La combinación por posición mantiene el ranking independiente
   * de la escala absoluta de cada señal.
   */
  const semanticRank = rankNormalizedScores(semanticScores).map(
    (score) => score ?? 0,
  );

  const lexicalRank = rankNormalizedScores(lexicalScores).map(
    (score) => score ?? 0,
  );

  const config = getDocumentConfig();
  const weightTotal =
    config.semanticWeight + config.lexicalWeight;

  const semanticWeight =
    weightTotal > 0
      ? config.semanticWeight / weightTotal
      : 0.7;

  const lexicalWeight =
    weightTotal > 0
      ? config.lexicalWeight / weightTotal
      : 0.3;

  const ranked = chunks
    .map((chunk, index) => {
      const hasSemanticScore =
        semanticScores[index] !== undefined;

      const score = embeddingUsed
        ? hasSemanticScore
          ? semanticWeight * semanticRank[index] +
    ...
    .sort(
      (left, right) =>
        ...
        (right.semanticScore ?? -Infinity) -
          (left.semanticScore ?? -Infinity) ||
  ...
  return {
    ...
    ...(result
      ? {
          ...
          lexicalScore: Number(
            result.lexicalScore.toFixed(4),
          ),
          ...(result.semanticScore === undefined
            ? {}
            : {
                semanticScore: Number(
                  result.semanticScore.toFixed(4),
                ),
              }),
        }
      : {}),
...
function selectedDocuments(
  ...
  return getDocuments(workspaceId, ids).sort(
    (left, right) => left.createdAt - right.createdAt,
  );
}

function estimatedContextCharacters(
  chunks: DocumentChunk[],
): number {
  return chunks.reduce(
    (total, chunk) =>
      total + contextBlock(chunk).length + 2,
    0,
  );
}
...
async function buildRag(
  ...
  for (const { chunk } of ranked) {
    ...
    if (block.length < fullBlock.length) {
      contextWasCut = true;
    }
...
async function buildCag(
  ...
  const config = getDocumentConfig();
  const documents = selectedDocuments(workspaceId, ids);
  const allChunks = documents.flatMap(
    (document) => document.chunks,
  );

  const completeContextFits =
    estimatedContextCharacters(allChunks) <=
    config.maxCagContextCharacters;

  const cacheQuery = completeContextFits
    ? undefined
    : query;

  const cached = getCachedCagContext(
    workspaceId,
    documents,
    cacheQuery,
  );
  ...
  if (truncated) {
    ...
    if (ranked.length > 0) {
      contextChunks = ranked.map((result) => result.chunk);
    }
  }
...
export async function buildKnowledgeContext(
  ...
  return mode === "rag"
    ? buildRag(workspaceId, query, ids)
    : buildCag(workspaceId, query, ids);
}