...
test("RAG conserva un candidato semántico sin coincidencias léxicas", async () => {
  const { addDocument } = await vite.ssrLoadModule("/lib/documents/store.ts");
  const { buildKnowledgeContext } = await vite.ssrLoadModule("/lib/documents/retrieval.ts");
  const workspaceId = crypto.randomUUID();
  const documentId = crypto.randomUUID();
  ...
  globalThis.fetch = async (_input, init) => {
    const body = JSON.parse(String(init?.body));
    const embeddings = body.input.length > 1
      ? body.input.map((text) => String(text).includes("autorización") ? [1, 0] : [0, 1])
      : [[1, 0]];
